    ...
    return (
        <>
            ...
            {/*                <div className="w-full h-1/5 flex justify-center items-center border-b">*/}
                                {!productDataIsLoading && !productError && isSeller(productData.seller , userData) && !productData.isSold &&
                                    (<Button value={"View Bids"} option={1} type={"button"} onClick={() => {setModalIsOpen(true)}}/>)}
            {/*                    {!isLoading && !error && !userIsSeller && isBidActive && !isSold &&*/}
            ...
            {/*                    {!isLoading && !error && !userIsSeller && !isBidActive && !isSold && finalBuyerSelected &&*/}
            {/*                        (<Button value={"Buy Now"} option={0} type={"button"}/>)}*/}
            {/*                    {!isLoading && !error && isSold &&*/}
