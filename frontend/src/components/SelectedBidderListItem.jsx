    ...
    return (
        <>
            <li className="relative w-[95%] p-3 mb-2 list-none rounded-lg border border-slate-300 bg-slate-100 shadow-lg shadow-slate-300 group transition ease-in-out duration-500 hover:scale-[101%] hover:-translate-y-1">
                ...
                <label htmlFor={bidderId} className="flex items-center text-lg p-1 cursor-pointer before:content-['+'] before:mr-2 before:font-semibold before:text-lg">{bidderName} <span className="font-medium absolute right-7"> Tk.{bid} </span></label>
                {isSelected && (
                    <>
                        <div className="max-h-0 overflow-hidden mt-2 mr-3.5 peer-checked:max-h-10 flex flex-row justify-end items-center transition-all ease-in-out duration-500 ">
