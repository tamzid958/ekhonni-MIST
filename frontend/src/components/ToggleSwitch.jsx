...
const ToggleSwitch = ({bidIsActive , onClick}) => {
    return (
        <label htmlFor="toggleButton" className="flex bg-slate-300 cursor-pointer relative w-32 h-12 rounded-md">
            <input checked={bidIsActive} onClick={onClick} type="checkbox" id="toggleButton" className="sr-only peer"/>
            <span className="w-[68px] h-10 bg-slate-500 absolute rounded-md left-14 top-1 peer-checked:bg-slate-900 peer-checked:left-1 transition-all ease-in-out duration-500">
